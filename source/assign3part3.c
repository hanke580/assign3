...
unsigned long SizeOfVirtualMemory;
unsigned long SizeOfPhysicalMemory;

char * sequenceFileName;
char * outputFileName;

void analyzeAccessSequenceFromFile(char * fileName, char * outfileName, int pageTableSize) {
	// Open this file 
	int fd;
	struct stat st;
...
int main(int argc, char ** argv) {
    ...
    SizeOfPhysicalMemory = strtoul(argv[3], NULL, 10);
	...
	analyzeAccessSequenceFromFile(sequenceFileName, outputFileName, pageTableSize);
